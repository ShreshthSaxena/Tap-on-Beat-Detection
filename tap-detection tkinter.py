...
if __name__ == "__main__":
	...
	root = Tk()
	root.title("rhythm drill")
	#root.geometry('350x200')
	lbl = ttk.Label(text="BPS (def 90)")
	lbl.grid(column=0, row=0)
	k1 = IntVar(value=90)
	k1entry = ttk.Entry(textvariable=k1)
	k1entry.grid(column = 1, row =0)
	k1entry.focus()
	lbl_streak = ttk.Label(text='0')
	lbl_streak.grid(column=2, row=1)
	def close_window():
		root.destroy()
	btn2 = ttk.Button(text="Stop", command = close_window)
		...
		for i in range(1000):
			...
			if tt.listen() == 1:
				...
				if b[c]-0.1 <= round(time.time()-start,2) <= b[c]+0.1:
					...
					c=0
	btn = ttk.Button(text="Play", command = clicked)
